...
def draw_chessboard(treshSavingFolder, finalSavingFolder, colors):
    ...
    for i in range(len(images)-1, -1, -1):
        ...
        color_img = cv2.cvtColor(cv2.imread(colorFiles[i]).copy(), COLOR_BGR2HSV)
        img     = cv2.imread(images[i])
        img    = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
...
def detect_color(colorFlolder, treshFolder):
    ...
    for i in range(len(treshFiles)-1, -1, -1):
        ...
        if len(cnts) != 0:
            for c in cnts:
                for c in cnts:
                    mask = np.zeros(color_img.copy().shape[:2], dtype="uint8")
                    cv2.drawContours(mask, [c], -1, 255, -1)
                    cv2.dilate(mask, None, iterations=2)
                    color = cv2.mean(color_img, mask=mask)[:3]
                    all_color_list.append([color[0], 0, 0])
